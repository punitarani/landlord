...
// Define types for Place and Review to match use-cached-places.ts
interface Review {
  id: string;
  place_id: string;
  rating: number;
...
export function useIndexedDB() {
  ...
  // Set cache timestamp
  const setCacheTimestamp = useCallback(
    async (key: string, expiryMinutes = 60) => {
  ...
  const isCacheValid = useCallback(
    async (key: string, maxAgeMinutes = 60): Promise<boolean> => {
      ...
      try {
        ...
        const timestampRecord = await new Promise<CacheTimestamp | undefined>(
          (resolve, reject) => {
        ...
        const isValid =
          now < timestampRecord.expiresAt &&
          now - timestampRecord.timestamp < maxAge;
  ...
  const storePlaces = useCallback(
    async (places: Place[], cacheDurationMinutes = 60) => {
      if (!db || !isReady) return;

      try {
        const transaction = db.transaction(
          [STORES.places, STORES.timestamps],
          "readwrite",
        );
  ...
  const storeReviews = useCallback(
    async (reviews: Review[], cacheDurationMinutes = 60) => {
      if (!db || !isReady) return;

      try {
        const transaction = db.transaction(
          [STORES.reviews, STORES.timestamps],
          "readwrite",
        );
    ...
    async (placeIds?: string[]) => {
      ...
      try {
        ...
        for (const placeId of placeIds) {
          const placeReviews = await new Promise<Review[]>(
            (resolve, reject) => {
  ...
  const clearCache = useCallback(async () => {
    ...
    try {
      const transaction = db.transaction(
        [STORES.places, STORES.reviews, STORES.timestamps],
        "readwrite",
      );
